 ...
 * @category   Plugin
 */
class CalcFixedPerProduct extends CalcAbstract {
	...
	/**
	 * CalcFixedPerProduct constructor.
	 *
	...
	public function calculate_discount(): float { // phpcs:ignore
		...
		if ( $quantity >= $discount_for_quantities ) {
			...
			if ( ! empty( $this->rule['get_quantity'] ) ) { // phpcs:ignore
				$discount_for_quantities  = (int) $this->rule['get_quantity'];
				$discount_for_quantities *= $multiplier;
			}
			$discount_amount = $discount_for_quantities * $fixed_per_product;
		}

		$this->discounted_quantities = $discount_for_quantities;

		return apply_filters( 'disco_final_discounted_amount', $discount_amount, 'fixed_per_product' );
	}
