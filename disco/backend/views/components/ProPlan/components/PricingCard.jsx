export default function PricingCard({ plan, yearly }) {
	...
	return (
		<div
			className={`disco:relative disco:rounded-2xl disco:border-2 disco:shadow-lg disco:hover:border-primary disco:p-6 disco:bg-white disco:transition-all ${plan.popular ? 'disco:border-emerald-400 disco:shadow-lg disco:shadow-emerald-100 disco:z-10' : 'disco:border-gray-200'}`}
		>
			{plan.popular && (
			...
			<button
				onClick={handleClick}
				className={`disco:w-full disco:py-2.5 disco:rounded-lg disco:font-semibold disco:text-sm  disco:transition-all ${plan.popular ? 'disco:bg-emerald-500 disco:text-white disco:hover:bg-primary' : 'disco:bg-[#F1F5F9] disco:border-[#E2E8F0] disco:hover:border-primary disco:text-[#334155] disco:hover:bg-primary disco:hover:text-white'}`}
			>
				⚡️ Get Started →
			</button>
