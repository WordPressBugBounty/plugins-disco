...
class Enqueue extends Base {
    ...
    public function enqueue_admin_styles() {
        ...
        if ( ! \is_null( $admin_page ) && in_array( $admin_page->id, array( 'disco_page_help-and-docs', 'disco_page_pro-plan', 'disco_page_compatible-plugin-list', 'disco_page_disco-analytics' ), true ) ) {
    ...
    public function enqueue_admin_scripts() {
        ...
        if ( ! \is_null( $admin_page ) && 'disco_page_disco-analytics' === $admin_page->id ) {
            $scripts[5] = new Script( DISCO_TEXTDOMAIN . '-admin-analytics-script', \plugins_url( 'assets/build/plugin-analytics.js', DISCO_PLUGIN_ABSOLUTE ) );
            $scripts[5]->forLocation( Asset::BACKEND )->withVersion( DISCO_VERSION );
            $scripts[5]->dependencies();
            $scripts[5]->withLocalize(
                'DISCO',
                array(
                    'is_pro_active' => Disco::is_pro(),
                    'json_url'      => esc_url_raw( rest_url( 'disco/v1' ) ),
