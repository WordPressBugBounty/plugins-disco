...
function PluginCard({ plugin }) {
	...
	return (
		<div className="disco:bg-white disco:border disco:border-[#10c88a] disco:rounded-[14px] disco:overflow-hidden disco:flex disco:flex-col disco:shadow-[0px_0px_0px_3px_rgba(245,158,11,0.07)]">
			{/* Top accent bar */}
			<div
				className="disco:h-0.75 disco:shrink-0"
				style={{
					backgroundImage:
			...
			{/* Card body */}
			<div className="disco:flex disco:flex-col disco:flex-1 disco:p-4.5">
				{/* Icon + status */}
				<div className="disco:flex disco:items-start disco:justify-between disco:mb-5">
					<div className="disco:w-12 disco:overflow-hidden disco:shrink-0">
				...
				{plugin.tags && plugin.tags.length > 0 && (
					<div className="disco:flex disco:flex-wrap disco:gap-1.25 disco:mt-auto disco:mb-3">
						{plugin.tags.map((tag) => (
							<span
								key={tag}
			...
			{/* Bottom action bar */}
			<div className="disco:border-t disco:border-[#f1f5f9] disco:px-4.5 disco:h-14 disco:flex disco:items-center disco:justify-between disco:gap-3 disco:shrink-0">
				{!isPro ? (
					<>
						<button
							className="disco:flex disco:items-center disco:gap-1.5 disco:h-7.75 disco:px-3.5 disco:rounded-[10px] disco:text-[12px] disco:font-bold disco:text-black disco:shrink-0 disco:hover:scale-105"
							style={{
								backgroundImage:
						...
						<a
							...
							className="disco:text-[11.5px] disco:font-semibold disco:text-[#2563eb]! disco:no-underline disco:hover:underline"
						>
					...
					<a
						...
						target="_blank"
						rel="noopener noreferrer"
						className="disco:w-full disco:text-xs disco:font-bold disco:text-primary-dark! disco:no-underline disco:bg-primary-light disco:rounded-lg disco:px-5 disco:py-2 disco:hover:scale-105 disco:hover:text-primary-dark! disco:transition-all! disco:duration-300 disco:text-center"
					>
						{__('📄 View Docs →', 'disco')}
					</a>
				)}
