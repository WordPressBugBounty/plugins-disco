...
const StatCard = ({
	...
	return (
		<div
			className={cn(
				'disco:relative disco:overflow-hidden disco:rounded-[10px] disco:border-2 disco:border-[#e5e7eb] disco:p-4 disco:flex-1 disco:min-w-60',
				CARD_STYLES[variant]
			)}
		>
			...
			{isLoading ? (
				...
				<>
					...
					{trend && (
						<Badge
							className={cn(
								'disco:text-[12px] disco:font-medium disco:rounded-full disco:px-2 disco:py-0.5 disco:border-0',
								isTrendUp
									? 'disco:bg-[#dcfce7]! disco:text-[#15803d]'
									: 'disco:bg-[#FFE0DE]! disco:text-[#DC5243]'
							)}
						>
							{trend}
						</Badge>
